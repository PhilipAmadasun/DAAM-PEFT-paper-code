...
model, lora_params = add_lora_layers(model.module if isinstance(model, nn.DataParallel) else model, target_modules, rank=4, alpha=16)
...
#94.87% for r=8 epoch 0
#Epoch 2/2, Train Loss: 0.1140, Val Accuracy: 0.9522 r=8, 6553600
...
def main():
    ...
    with open("run3_r4alpha16.txt", 'a') as f:
        string = f"Best validation accuracy rate: {best_acc}, Number of trainable parameters: {number}"
        f.write(string + "\n")

    model_save_path = "./model_saver4alpha16"
    tokenizer_save_path = "./tokenizer_saver4alpha16"
