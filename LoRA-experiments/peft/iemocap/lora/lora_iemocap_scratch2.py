...
def add_lora_layers(model, target_modules, rank=4, alpha=32):
    lora_params = []
...
def main():
    ...
    target_modules = ['q_proj', 'v_proj']  # This is similar to the original script
    model, lora_params = add_lora_layers(model, target_modules, rank=8, alpha=16)  # Adjust rank as needed
    ...
    for fold in range(5):
        ...
        with open("final_results_r8alpha16.txt", 'a') as f:
            string = "Fold "+str(fold)+", Best validation accuracy rate: "+str(best_acc) + ", Number of trainable parameters:" + str(number) 
            f.write(string)
            f.write("\n")
