...
set_seed(111)  # You can choose any seed value
...
class DAAMModel(nn.Module):
    def __init__(self, num_classes, num_heads=8, dropout_rate=0.15):
        super(DAAMModel, self).__init__()

        self.gct = MultiHeadDAAM(num_layers=40, time_steps=1, feature_size=5120, num_heads=num_heads)
        self.gqa = MultiheadGQA(embed_dim=512, query_heads=8, kv_heads=2, device="cuda", dtype=torch.float32)

        self.conv1 = nn.Conv2d(in_channels=40, out_channels=512, kernel_size=(3,3), stride=1, padding=1)
        self.conv2 = nn.Conv2d(in_channels=512, out_channels=40, kernel_size=(3,3), stride=1, padding=1)

        self.embed_transform = nn.Linear(5120, 512)

        self.fc = nn.Linear(40 * 512, num_classes)  # Fully connected layer
    ...
    def forward(self, x, save_attention_weights=False):
        ...
        if save_attention_weights:
            ...
            x, attention_weights_ = self.gct(x, save_attention_weights)
        
        else:

            x = self.gct(x, save_attention_weights)
        ...
        out, attn_weights = self.gqa(query, key, value, need_weights=True)
        
        batch_size = out.size(0)
        num_features = 40  # This should match the in_channels of self.conv1
        height = 64
        width = 8
        x = out.view(batch_size, num_features, height, width)
...
def train_and_validate(model, train_loader, val_loader, optimizer, criterion, scheduler, num_epochs, device, start_epoch=0, best_val_accuracy=0.0, fold_dir=''):
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        if val_accuracy > best_val_accuracy:
            ...
            with open(os.path.join(fold_dir,'GQADAAMlog.txt'), 'a') as f:
...
# params: 3,533,352
model = DAAMModel(num_classes=4)  # Adjust the number of classes as needed

#print(count_trainable_parameters(model))


# Define the path to the pickle files
train_pickle_path = '../train_dataset.pkl'  # Update with the actual path
val_pickle_path = '../val_dataset.pkl'      # Update with the actual path

# Load datasets
train_dataset = AGNewsDataset(train_pickle_path)
val_dataset = AGNewsDataset(val_pickle_path)
...
model.to(device)
optimizer = optim.AdamW(model.parameters(), lr=1e-4 / 2.0)
scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'max', factor=0.1, patience=10, verbose=True)
#criterion = FocalLoss()  # Using the FocalLoss defined earlier
