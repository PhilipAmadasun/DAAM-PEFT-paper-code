...
class MHAModel(nn.Module):
    def __init__(self, num_classes, num_heads=8, dropout_rate=0.15):
        ...
        self.gct = MultiheadAttention(embed_dim=5120, num_heads=num_heads)
        ...
        self.fc = nn.Linear(40 * 5120, num_classes)  # Fully connected layer
    ...
    def forward(self, x, save_attention_weights=False):
        ...
        if save_attention_weights:
            ...
            x, attention_weights_ = self.gct(x,x,x)#, save_attention_weights)
        
        else:

            x, _ = self.gct(x,x,x)# save_attention_weights)
        

        x = x.reshape(x.size(0), x.size(1), 64, 80)
...
def train_and_validate(model, train_loader, val_loader, optimizer, criterion, scheduler, num_epochs, device, start_epoch=0, best_val_accuracy=0.0, fold_dir=''):
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        if val_accuracy > best_val_accuracy:
            ...
            with open(os.path.join(fold_dir,'MHAlog.txt'), 'a') as f:
...
model = MHAModel(num_classes=4)  # Adjust the number of classes as needed
